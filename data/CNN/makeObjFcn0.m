...
% 5. 返回已保存网络的验证误差和文件名。
function ObjFcn = makeObjFcn0(XTrain,YTrain,XValidation,YValidation,XTest,YTest,DATARoot)
ObjFcn = @valErrorFun;
    ...
    function [valError,cons,fileName] = valErrorFun(optVars)
        ...
        maxEpochs = 300; validationPatience = 150;
...
%         绘制混淆矩阵
        ...
        cm.Title = sprintf('%d _ %.2f', numFile, (1-valError) * 100);
        cm.ColumnSummary = 'column-normalized';
        cm.RowSummary = 'row-normalized';
        saveas(cm, fullfile(DATARoot,['CM_' num2str(numFile) '_' num2str(valError)  '.jpg']))
        close(figcm)
%         绘制每个训练图像中验证集Loss图像
