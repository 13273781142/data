...
optimVars = [
    ...
    optimizableVariable('HiddenUnits',[20 40],'Type','integer')
    optimizableVariable('DropOut',[0.10 0.99],'Transform','log')
...
% 4. 贝叶斯优化
% 4.1 输入数据集调用
ObjFc = makeObjFcnLSTM(XTRA,YTRA,XVAL,YVAL,dataDir);
% 4.2 运行贝叶斯优化
...
testError = (cell2mat(YTES) - cell2mat(YPredicted)).^2;
rmseError = mean( sqrt(sum(testError, 2) / numClasses) );
% 5.2.3 打印信息
...
% 6.2 保存预测图
for yRows = 100:105
    figcm = figure('Name','YC','Units','centimeters','Position',[5, 5, 17, 8.5]);
    YPredPlot = plot( cell2mat(YPredicted(yRows)), '--');
    hold on
    YLabelPlot = plot( cell2mat(YTES(yRows)) );
