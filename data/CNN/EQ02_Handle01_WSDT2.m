...
%% Reshape
% 数据形状-4D
datastra = reshape(datastra, lenaccelh, lenaccelw, numInputChannels, size(datastra,2));
labeltra = categorical(labeltra);
datasval = reshape(datasval, lenaccelh, lenaccelw, numInputChannels, size(datasval,2));
labelval = categorical(labelval);
datastes = reshape(datastes, lenaccelh, lenaccelw, numInputChannels, size(datastes,2));
